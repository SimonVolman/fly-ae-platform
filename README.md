...
3 GiB через OTP, доставляемый по email. Оба проходят
...
Вход выполняется только по email. Telegram-бот используется для уведомлений
о загрузках и открытии share-ссылок, а также для приватных команд администратора.
Старые ссылки входа в Telegram больше не создают сессии: бот предлагает войти
по email на сайте. Webhook по-прежнему проверяет
...
`FLY_TELEGRAM_ENABLED` управляет работой бота, а не способом входа.
